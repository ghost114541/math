...
- 同时显示函数形式和齐次矩阵形式。
- 区分两个概念：
  - Geometric one-point perspective transform；
...
- 页面中说明和 three.js 的对应关系：
  - `M_model` 接近 `object.matrixWorld`；
  - `M_view` 对应 `camera.matrixWorldInverse`；
  - shader 中的 `modelViewMatrix` 对应二者的乘积。
...
- OpenGL 4.6 Specification / Khronos vertex post-processing documentation。

教学和实现资料：

- MDN WebGL model-view-projection 文档。
- LearnOpenGL Coordinate Systems。
- Scratchapixel perspective projection matrix 和 clipping 资料。
- Microsoft View Transformation 文档。
- MDPI 关于 3D line clipping algorithms 的文章。
